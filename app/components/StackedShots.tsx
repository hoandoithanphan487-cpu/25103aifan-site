...
type StackedShotsProps = {
  shots: readonly Shot[];
  /** Valley sits opposite Moonshadow, so the pile leans the other way. */
  mirror?: boolean;
};

export function StackedShots({ shots, mirror = false }: StackedShotsProps) {
  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={cycle}
        aria-label={`下一张界面：${shots[(front + 1) % n]?.label ?? ""}`}
      ...
      <p
        aria-live="polite"
        className="px-[9%] font-editorial text-[0.6rem] uppercase tracking-[0.2em] text-ink-faint"
      >
        {current.label}
      </p>

      <p className="group/hint relative mt-4 inline-flex flex-col px-[9%] text-ink-faint">
